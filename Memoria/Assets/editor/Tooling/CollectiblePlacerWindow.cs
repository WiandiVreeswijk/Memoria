...
public class CollectiblePlacerWindow : EditorWindow {
    [Serializable]
    ...
    public static void ShowWindow() {
        EditorWindow.GetWindowWithRect(typeof(CollectiblePlacerWindow), new Rect(0, 0, 300, 200));
    }
    ...
    public void OnEnable() {
        Initialize(false);
        var data = EditorPrefs.GetString("CollectiblePlacer", JsonUtility.ToJson(this, false));
        JsonUtility.FromJsonOverwrite(data, this);
        Collectibles collectibles = Utils.FindUniqueObject<Collectibles>();
        if (collectibles != null) parent = collectibles.transform;
    ...
    protected void OnDisable() {
        ...
        EditorPrefs.SetString("CollectiblePlacer", data);
        SceneView.duringSceneGui -= DuringSceneGui;
    }
    ...
    private Vector3 FreeMoveHandle(Vector3 pos, Color color) {
        ...
        Handles.color = color;
        Handles.zTest = UnityEngine.Rendering.CompareFunction.LessEqual;
        toRet = Handles.FreeMoveHandle(pos, Quaternion.identity, 0.2f, new Vector3(0.25f, 0.25f, 0), Handles.SphereHandleCap);
        Handles.color = Color.gray;
        Handles.zTest = UnityEngine.Rendering.CompareFunction.Greater;
        toRet = Handles.FreeMoveHandle(pos, Quaternion.identity, 0.2f, new Vector3(0.25f, 0.25f, 0), Handles.SphereHandleCap);
        Handles.zTest = UnityEngine.Rendering.CompareFunction.Always;
    ...
    public void OnGUI() {
        ...
        if (GUILayout.Button("Place Points", GUILayout.Height(30.0f))) {
